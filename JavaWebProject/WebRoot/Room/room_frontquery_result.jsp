...
<%
    ...
    String roomNo = (String)request.getAttribute("roomNo"); //房间编号查询关键字
    ...
    String roomState = (String)request.getAttribute("roomState"); //房间状态查询关键字
%>
<!DOCTYPE html>
<html>
<head>
	...
	<div class="col-md-9 wow fadeInLeft">
		...
		<div class="row">
			...
			<div class="col-md-3 bottom15" <%=clearLeft %>>
			  <a  href="<%=basePath  %>Room/<%=room.getRoomNo() %>/frontshow"><img class="img-responsive" src="<%=basePath%><%=room.getMainPhoto()%>" /></a>
			     <div class="showFields">
			     	<div class="field">
	            		房间编号:<%=room.getRoomNo() %>
			     	</div>
			     	<div class="field">
	            		房间类型:<%=room.getRoomTypeObj().getTypeName() %>
			     	</div>
			     	<div class="field">
	            		房间名称:<%=room.getRoomName() %>
			     	</div>
			     	<div class="field">
	            		房间价格:<%=room.getPrice() %>
			     	</div>
			     	<div class="field">
	            		房间状态:<%=room.getRoomState() %>
			     	</div>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="roomQueryForm" id="roomQueryForm" action="<%=basePath %>Room/frontlist" class="mar_t15" method="post">
			<div class="form-group">
				...
				<input type="text" id="roomNo" name="roomNo" value="<%=roomNo %>" class="form-control" placeholder="请输入房间编号">
			</div>
            <div class="form-group">
            	<label for="roomTypeObj_typeId">房间类型：</label>
                <select id="roomTypeObj_typeId" name="roomTypeObj.typeId" class="form-control">
                	<option value="0">不限制</option>
	 				<%
	 				for(RoomType roomTypeTemp:roomTypeList) {
	 					String selected = "";
 					if(roomTypeObj!=null && roomTypeObj.getTypeId()!=null && roomTypeObj.getTypeId().intValue()==roomTypeTemp.getTypeId().intValue())
 						selected = "selected";
	 				%>
 				 <option value="<%=roomTypeTemp.getTypeId() %>" <%=selected %>><%=roomTypeTemp.getTypeName() %></option>
	 				<%
	 				}
	 				%>
 			</select>
			...
			<div class="form-group">
				<label for="roomState">房间状态:</label>
				<input type="text" id="roomState" name="roomState" value="<%=roomState %>" class="form-control" placeholder="请输入房间状态">
			</div>
