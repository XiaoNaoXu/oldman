...
<jsp:include page="../header.jsp"></jsp:include>
		...
		<div class="col-md-12 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" ><a href="<%=basePath %>RoomType/roomType_frontAdd.jsp" style="display:none;">添加房间类型</a></li>
				</ul>
			  	<!-- Tab panes -->
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>房间类型id</td><td>房间类型名称</td><td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
				    	            		...
				    	            		for(int i=0;i<roomTypeList.size();i++) {
					    	            		...
					    	            		RoomType roomType = roomTypeList.get(i); //获取到房间类型对象
 										%>
 										<tr>
 											<td><%=currentIndex %></td>
 											...
 											<td>
 												<a href="<%=basePath  %>RoomType/<%=roomType.getTypeId() %>/frontshow"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="roomTypeEdit('<%=roomType.getTypeId() %>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="roomTypeDelete('<%=roomType.getTypeId() %>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
	...
	<div style="display:none;">
		...
		<form name="roomTypeQueryForm" id="roomTypeQueryForm" action="<%=basePath %>RoomType/frontlist" class="mar_t15" method="post">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
			 <label for="roomType_typeId_edit" class="col-md-3 text-right">房间类型id:</label>
			 <div class="col-md-9"> 
			 	<input type="text" id="roomType_typeId_edit" name="roomType.typeId" class="form-control" placeholder="请输入房间类型id" readOnly>
			 </div>
		  </div> 
		  <div class="form-group">
		  	 <label for="roomType_typeName_edit" class="col-md-3 text-right">房间类型名称:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="roomType_typeName_edit" name="roomType.typeName" class="form-control" placeholder="请输入房间类型名称">
			 </div>
		  </div>
		</form> 
...
/*弹出修改房间类型界面并初始化数据*/
function roomTypeEdit(typeId) {
	$.ajax({
		url :  basePath + "RoomType/" + typeId + "/update",
		type : "get",
		dataType: "json",
...
/*删除房间类型信息*/
function roomTypeDelete(typeId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url : basePath + "RoomType/deletes",
			data : {
				typeIds : typeId,
			},
...
function ajaxRoomTypeModify() {
	$.ajax({
		url :  basePath + "RoomType/" + $("#roomType_typeId_edit").val() + "/update",
		type : "post",
		dataType: "json",
