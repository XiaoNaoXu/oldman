...
public class RoomTypeService {
    ...
    /*根据主键获取房间类型记录*/
    public RoomType getRoomType(int typeId) throws Exception  {
        RoomType roomType = roomTypeMapper.getRoomType(typeId);
        return roomType;
    }
    ...
    /*删除一条房间类型记录*/
    public void deleteRoomType (int typeId) throws Exception {
        roomTypeMapper.deleteRoomType(typeId);
    }

    /*删除多条房间类型信息*/
    public int deleteRoomTypes (String typeIds) throws Exception {
    	String _typeIds[] = typeIds.split(",");
    	for(String _typeId: _typeIds) {
    		roomTypeMapper.deleteRoomType(Integer.parseInt(_typeId));
    	}
