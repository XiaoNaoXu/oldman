...
public class RoomTypeController extends BaseController {
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(RoomType roomType:roomTypeList) {
			...
			jsonRoomType.accumulate("typeId", roomType.getTypeId());
			jsonRoomType.accumulate("typeName", roomType.getTypeName());
			jsonArray.put(jsonRoomType);
		}
	...
	public void delete(String typeIds,HttpServletRequest request,HttpServletResponse response) throws IOException, JSONException {
    	...
    	boolean success = false;
        try { 
        	int count = roomTypeService.deleteRoomTypes(typeIds);
        	success = true;
	...
	public void OutToExcel( Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "房间类型id","房间类型名称"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<roomTypeList.size();i++) {
        	RoomType roomType = roomTypeList.get(i); 
        	dataset.add(new String[]{roomType.getTypeId() + "",roomType.getTypeName()});
        }
        /*
